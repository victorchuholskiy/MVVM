...
class CipherWrapper(transformation: String) {
	...
	fun encrypt(data: String,
				key: Key?,
				useInitializationVector: Boolean = false): String {
	...
	fun wrapKey(keyToBeWrapped: Key,
				keyToWrapWith: Key?): String {
	...
	fun unWrapKey(wrappedKeyData: String,
				  algorithm: String,
				  wrappedKeyType: Int,
				  keyToUnWrapWith: Key?): Key {
